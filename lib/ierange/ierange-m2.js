 ...
 */
    ...
    var TextRangeUtils = {
        ...
        convertFromDOMRange: function (domRange) {
            function adoptEndPoint(textRange, domRange) {
                // find anchor node and offset
                var container = domRange['endContainer'];
                var offset = domRange['endOffset'];
                var textOffset = 0;
                ...
                cursorNode.parentNode.removeChild(cursorNode);
                // move range
                textRange.setEndPoint('EndToStart', cursor);
                textRange['moveEnd']('character', textOffset);
            }
            function adoptStartPoint(textRange, domRange) {
                // find anchor node and offset
                var container = domRange['startContainer'];
                ...
                cursorNode.parentNode.removeChild(cursorNode);
                // move range
                textRange.setEndPoint('StartToStart', cursor);
                textRange['moveStart']('character', textOffset);
            }
		
            // return an IE text range
            var textRange = domRange._document.body.createTextRange();
            adoptEndPoint(textRange, domRange);
            adoptStartPoint(textRange, domRange);
            return textRange;
        }
    };
